    ...
    if (placeId.isNotEmpty) {
      try {
        ...
        if (mounted) {
          ...
          try {
            final category = (widget.place['category']?.toString() ?? 'unknown')
                .toLowerCase();
            final placeId = widget.place['_id']?.toString() ??
                widget.place['id']?.toString() ??
                '';
...
class _BookingDialogState extends State<BookingDialog> {
  ...
  Future<void> _confirmBooking() async {
    ...
    try {
      ...
      // Use BookingService to create actual booking in database
      Map<String, dynamic> result;
      bool usedMockService = false;

      try {
        print('🎯 Creating booking using BookingService...');
