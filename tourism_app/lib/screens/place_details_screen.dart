...
// import 'package:tourism_app/providers/user_behavior_provider.dart'; // Commented out - file not found
...
class _PlaceDetailsScreenState extends State<PlaceDetailsScreen>
  ...
  // UserBehaviorProvider? _userBehaviorProvider; // Commented out - provider not found
  ...
  void initState() {
    ...
    _imageGallery = _getPlaceImages(widget.place['image_path'] as String?);
  ...
  List<String> _getPlaceImages(String? mainImagePath) {
    // Fallback to app logo if image path is null/empty
    if (mainImagePath == null || mainImagePath.trim().isEmpty) {
      return ['assets/logo.png'];
    }

    final String normalized = mainImagePath.trim();
    String fullMainPath = normalized.startsWith('assets/')
        ? normalized
        : 'assets/places/$normalized';

    List<String> images = [fullMainPath];

    // Try to find related images for the same place
    String baseName = normalized.split('.').first;
    ...
    // Ensure we have at least one image; if loading fails later, errorBuilder will handle UI
    if (images.isEmpty) {
  ...
  void didChangeDependencies() {
    ...
    //     Provider.of<UserBehaviorProvider>(context, listen: false); // Commented out - provider not found
  }

  @override
  void dispose() {
    final seconds = DateTime.now().difference(_enterTime).inSeconds.toDouble();
    // _userBehaviorProvider?.recordViewTime(seconds, notify: false); // Commented out - provider not found
...
// <CHANGE> Updated BookingDialog to use real payment service logic
  ...
  Future<void> _confirmBooking() async {
    ...
    try {
      ...
      try {
        ...
        // If booking service fails without a definitive payment status, try payment service directly
        final Map<String, dynamic>? bookingData =
            (result['data'] is Map<String, dynamic>) ? result['data'] : null;
        final String? bookingPaymentStatus = bookingData != null
            ? (bookingData['paymentStatus']?.toString().toLowerCase())
            : null;

        final bool shouldFallbackToPaymentService = (!result['success']) &&
            (bookingPaymentStatus == null || bookingPaymentStatus.isEmpty);

        if (shouldFallbackToPaymentService) {
          print(
              'BookingService failed without payment status, trying payment service');
        ...
        print('BookingService error, trying payment service: $e');
        // Only try direct payment if widget is still mounted and the error is not a user cancellation
        if (!mounted) return;
        result = await PaymentService.createPayment(
          userId: user?['_id'] ?? user?['id'] ?? '',
          userFullName: _nameController.text.trim(),
      ...
      if (result['success']) {
        ...
        if (mounted) {
          ...
          const successStatuses = {'confirmed', 'completed', 'paid', 'success'};
          const cancelStatuses = {'cancelled', 'canceled', 'user_cancelled'};

          if (successStatuses.contains(normalizedStatus) ||
              (normalizedStatus.isEmpty && (result['success'] == true))) {
            bool hasHormuudPayment =
                data['waafiResponse'] != null || data['hormuudPayment'] == true;
            _showPaymentSuccessDialog(
                data, actualPaidAmount, false, hasHormuudPayment);
          } else if (cancelStatuses.contains(normalizedStatus) ||
              data['paymentError']?['errorType'] == 'USER_CANCELLED') {
            if (!mounted) return;
            Navigator.of(context).pop(); // close dialog if still open
            _showErrorSnackBar('Payment was cancelled. You can try again.');
          } else {
            _showErrorSnackBar(
        ...
        // Treat failed response that contains a booking/payment status
        final data = result['data'] ?? {};
        final normalizedStatus = (data['bookingStatus'] ?? data['status'] ?? '')
            .toString()
            .toLowerCase();
